...
for (const [target, usbName] of variants) {
  ...
  if (info.status !== 0 || !info.stdout.includes(`name:              ${target}`) ||
      !info.stdout.includes("sdk version:       2.3.0") ||
      !info.stdout.includes("pico_board:        pico")) {
    throw new Error(`picotool validation failed for ${uf2Path}.`);
  }
  uf2Files.push(uf2);
  console.log(`Pico: ${target}.uf2 is RP2040/pico SDK 2.3.0 with USB name ${usbName}`);
}
